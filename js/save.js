        ...
        if (mode === 'new') {
          if (data) {
            this.requestConfirm(`Slot ${i} is occupied. Delete existing data for "${data.cafeName}" to start a new game?`, () => {
              this.deleteSlot(i);
              this.requestNamePrompt((newName) => this.startNew(i, newName));
            });
          } else {
...
// Global startGame bridge for SaveSystem
function startGame(isLoad, data, name) {
  if (typeof UIManager === 'undefined' || !UIManager.startGame) {
    console.error('UIManager not ready. Ensure ui.js loads before save.js.');
    return;
  }
  UIManager.startGame(isLoad, data, name);
}
